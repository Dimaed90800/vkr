...
def test_legacy_judge_input_does_not_count_as_wrapper_missing(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("DAST_DIAGNOSTIC_LOG_DIR", str(tmp_path / "logs"))
    task = _rate_abuse_task()
    legacy_evidence = {
        "response_summary": {"status": "cannot_proceed"},
        "indicators": [],
    }
    ...
    TaskScheduler().update_queue_after_verdict(
        ...
        max_retries=1,
        state=SchedulerState(run_id="run-judge-handoff-legacy", root_trace_id="run-judge-handoff-legacy"),
        fairness=FairnessConfig(),
    )

    events_path = tmp_path / "logs" / "run-judge-handoff-legacy" / "events.jsonl"
    events = [json.loads(line) for line in events_path.read_text(encoding="utf-8").splitlines() if line.strip()]
    event_types = {event["event_type"] for event in events}
    assert "judge_input_wrapper_fields_not_applicable" in event_types
    assert "judge_input_wrapper_fields_missing" not in event_types
