...
def main() -> int:
    ...
    payload = {
        ...
        'signals': ['akto_runtime_adapter', 'authz_runtime_plan_ready', 'runtime_inventory_built', 'planner_only'],
        'raw_report_paths': [str(report_path)],
        'candidate_findings': [candidate],
        'used_requests': 0,
        'termination_reason': 'planner_only',
        'fallback_reason': 'Akto upstream runtime is not installed; adapter produced an authorization plan only.',
        'error': None,
    }
    _write_json(args.result_json, payload)
    return 0
