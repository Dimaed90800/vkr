...
class TaskExecutabilityService:
    ...
    def _baseline_valid(self, task: TaskModel) -> bool | None:
        hints = task.context_hints or {}
        if self._can_test_without_valid_baseline(task):
            return True
    ...
    def _can_test_without_valid_baseline(self, task: TaskModel) -> bool:
        ...
        if task.class_name == "authorization" and subtype in {"property_level_authorization", "mass_assignment"}:
            return bool(task.params.body_fields or task.params.query_params) and (preferred == "property_mutation_test" or "property_mutation_test" in allowed)
