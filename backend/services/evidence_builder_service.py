...
class EvidenceBuilderService:
    def from_wrapper_result(
        ...
        *,
        ...
        planner_only = self._is_planner_only(tool_result=tool_result, signals=signals)
        strong_indicators = [] if planner_only else self._strong_security_indicators(signals)
    ...
    def _generic_wrapper_enrichment(
        ...
        *,
        ...
        planner_only = (
            "planner_only" in signal_set
            or str(tool_result.termination_reason or "").strip().lower() == "planner_only"
            or int(getattr(tool_result.budget, "used_requests", 0) or 0) == 0
        )
        if (
            not planner_only
            and (
                vuln_type in {"generic_access_control", "horizontal_privilege_escalation", "object_authorization", "bola", "vertical_privilege"}
                or class_like in {"generic_access_control", "bola", "vertical_privilege", "function_level_authorization"}
            )
        ):
    ...
    def _is_planner_only(self, *, tool_result: ToolWrapperResult, signals: list[str]) -> bool:
        signal_set = {str(item or "").strip().lower() for item in signals or []}
        termination = str(tool_result.termination_reason or "").strip().lower()
        return "planner_only" in signal_set or termination == "planner_only"
