...
class AuthPreparationService:
        ...
        for candidate in create_candidates[:3]:
            ...
            for body in body_variants:
                for content_type in media_type_variants:
                    attempt_index += 1
                    prepared_body = self._prepare_materialization_body_for_content_type(
                        candidate_path=candidate_path,
                        body=body,
                ...
                self._emit_prep_event(
                    ...
                    artifacts={
                        ...
                        "request_body_keys": sorted(list((prepared_body or {}).keys())) if isinstance(prepared_body, dict) else [],
    ...
    def _prepare_materialization_body_for_content_type(
        self,
        *,
        candidate_path: str,
        ...
        lowered_path = str(candidate_path or "").lower()
        if content_type != "multipart/form-data":
            return normalized
        if "/videos" in lowered_path and not any(key in normalized for key in {"file", "video", "upload", "videoFile", "video_file"}):
            normalized["file"] = (
                f"autodast-{uuid.uuid4().hex[:6]}.mp4",
                b"autodast-video-bytes",
                "video/mp4",
            )
        return normalized
